...
- (GDataXMLElement *)elementForName:(NSString *)name {
	NSArray *nodes = [self elementsForName:name];
...
- (GDataXMLElement *)elementWithAttribute:(NSString *)name attr:(NSString *)attrName attrValue:(NSString *)attrValue {
	GDataXMLElement *el;
	
	NSArray *nodes = [self elementsForName: @"link"];
	if (nil != nodes && [nodes count] > 0) {
		for (GDataXMLElement *node in nodes) {
			NSString *rel = [[node attributeForName: attrName] stringValue];
			if (rel == attrValue) {
				el = node;
				break;
			}
